    ...
    with wandb.init(project= args.name, config=vars(args), sync_tensorboard=True, mode='offline', group="lr=2e-5,epoch=600,early_stopping,", job_type=job_type):
        ...
        if args.mode == 0:
            for data_seed in range(loop):
                ...
                if args.cross_val == 'True':
                    if not args.augment:
                        train_dataset = dataset.Feature_bag_dataset(root=path, csv_path=data_csv, split_path=args.split_path, fold_num=data_seed, split="train", num_classes = args.n_classes, class_name = args.class_name, augment=args.augment)
                    else:
                        train_dataset = dataset.Feature_bag_dataset(root=path, csv_path=data_csv, split_path=args.split_path, fold_num=data_seed, split="train", num_classes = args.n_classes, class_name = args.class_name, augment=args.augment, result_dir=args.result_dir_teacher)
                ...
                for model_seed in range(model_seeds):
                    ...
                    for epoch in range(args.epochs):
                        if args.model == "CLAM-SB" or args.model == "CLAM-MB":
                            ...
                            train_loop_clam(epoch,model,train_dataloader,optimizer,n_classes=args.n_classes,bag_weight=args.bag_weight,writer=writer,device=device, mode=args.mode, augment=args.augment)
                            print(f"Starting Validation {epoch}")
                            stop = validate_clam(epoch,model,val_dataloader,n_classes=args.n_classes,writer=writer,device=device,early_stopping=early_stopping, results_dir =result_dir, mode=args.mode, augment=args.augment)
                    ...
                    if args.early_stopping:
                        ...
                        if args.augment:
                            eval_model.load_state_dict(torch.load(os.path.join(result_dir, "student_model.pt")))
                        else:
                            eval_model.load_state_dict(torch.load(os.path.join(result_dir, "model.pt")))
                    else:
                        if args.augment:
                            torch.save(model.state_dict(), os.path.join(result_dir, "student_model.pt"))
                        else:
            ...
            for data_seed in range(loop):
                ...
                for model_seed in range(model_seeds):
                    ...
                    eval_model.to(device)
                    # Manually copy the saved model over to the location below
                    shutil.move(os.path.join(result_dir, "model.pt"), os.path.join(result_dir, "teacher_model.pt"))
            ...
            for data_seed in range(loop):
                ...
                train_dataset = dataset.Feature_bag_dataset(root=path, csv_path=data_csv, split_path=args.split_path, fold_num=data_seed, split="train", num_classes = args.n_classes, class_name = args.class_name)
                # weights = make_weights_for_balanced_classes_split(train_dataset)
                # train_dataloader = DataLoader(train_dataset, num_workers=1, sampler = SequentialSampler(train_dataset))
                for model_seed in range(model_seeds):
                    seed_torch(model_seed,device)
                    print(f"Exp/fold:{data_seed}_{model_seed}")
                    result_dir = os.path.join(args.result_dir,'exp_'+str(data_seed)+'_'+str(model_seed))
