from MMIL_utils.core import *
from MMIL_utils.utils import *
import torch
import random
...
class MultipleMILTransformer(nn.Module):
    ...
    def forward(self,data, label=None,coords=False,mask_ratio=0):
        #---> init
        x = data
...
class Args:
    def __init__(self):
        self.in_chans = 1024  # Example value, adjust as needed
        self.embed_dim = 512  # Example value, adjust as needed
        self.n_classes = 3  # Example value, adjust as needed
        self.num_msg = 0  # Example value, adjust as needed
        self.num_subbags = 64  # Example value, adjust as needed
        self.mode = 'random'  # Example value, choose from ['random', 'coords', 'seq', 'embed', 'idx']
        self.num_layers = 2  # Example value, adjust as needed
        self.ape = False  # Example value, set to True or False as needed

# Sample usage of the MultipleMILTransformer class
def main():
    # Initialize arguments
    device = torch.device("cpu")
    args = Args()

    # Initialize model with the defined arguments
    model = MultipleMILTransformer(args)

    # Create a sample input tensor (adjust size as needed for your use case)
    ...
    # Forward pass through the model (adjust parameters as needed)
    logits, Y_prob, Y_hat, results_dict = model(data= data, label = labels)
    print("Logits:", logits)
    print("Y_prob:", Y_prob)
    print("Y_hat:", Y_hat)
